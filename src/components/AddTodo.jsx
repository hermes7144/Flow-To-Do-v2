...
import { getDeadline, isString } from '../js/CommonFunction';
...
export default function AddTodo({ category }) {
  ...
  const handleSubmit = (e) => {
    ...
    const todo = {
      name: input,
      deadline: isString(category) ? getDeadline(category) : null,
      estimate,
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className='flex my-3 bg-white items-center p-2 rounded-lg'>
        <input className='w-44 xs:flex-1 focus:outline-none' type='text' value={input} onChange={handleChange} placeholder={`+ "${isString(category) ? category : category.name} 할 일" 추가`} />
